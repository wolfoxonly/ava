...
const std::string strMessageMagic = "avaloncoin Signed Message:\n";
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool, bool fAddToMempool)
{
    ...
    if (chainActive.Tip()) mempool.removeForReorg(pcoinsTip, chainActive.Tip()->nHeight + 1, STANDARD_LOCKTIME_VERIFY_FLAGS);
...
static void CheckForkWarningConditions()
{
    ...
    if (pindexBestForkTip || (false && pindexBestInvalid && pindexBestInvalid->nChainWork > chainActive.Tip()->nChainWork + (GetBlockProof(*chainActive.Tip()) * 6)))
    {
