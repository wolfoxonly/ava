...
if [ x = x"$PLAT" ]; then echo "No platform specified, e.g. bitcoind/Win32Gui/Cross-Mac, case-sensitive"; exit 1; fi
if [ x = x"$COIN" ]; then echo "No COIN specified, e.g. package name in configure.ac, case-sensitive"; exit 1; fi
...
# Win32
	...
	export GOAL="install" 
	export BITCOIN_CONFIG="--disable-tests --disable-debug --disable-bench --without-libs --enable-reduce-exports"
